        ...
        <div class="max-w-lg p-6 bg-white rounded-lg shadow-xs dark:shadow-none dark:border dark:border-gray-700 dark:bg-gray-800">
            <form method="POST" action="{{ route('admin.kelas.update', $kelas) }}">
                ...
                <label class="block text-sm mb-4">
                    ...
                    <select name="jurusan_id" id="jurusan"
                        ...
                        @foreach($jurusanList as $j)
                            <option value="{{ $j->id_jurusan }}" {{ old('jurusan_id', $kelas->jurusan_id) == $j->id_jurusan ? 'selected' : '' }}>
                ...
                <label class="block text-sm mb-4">
                    ...
                    <input type="text" name="nomor_kelas" id="nomor_kelas"
        ...
        $(document).ready(function() {
            function updateNomorKelas() {
                ...
                $.get('{{ route('admin.kelas.next-nomor') }}', {
                    ...
                    if (oldVal && oldVal !== '{{ $kelas->nomor_kelas }}') return;
                    if (data.next_nomor) {
