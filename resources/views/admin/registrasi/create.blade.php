            ...
            <form method="POST" action="{{ route('admin.registrasi.store') }}">
                        ...
                        <select name="kelas_id"
                            ...
                            @foreach($kelas as $k)
                                <option value="{{ $k->id_kelas }}">
                                    {{ $k->nama_kelas }}
                                    ({{ $k->tahunAjaran->nama_tahun }} - {{ $k->tahunAjaran->semester }})
                                </option>
                            @endforeach
                        </select>
                        @error('kelas_id')
                ...
                <div class="mb-4">
                    ...
                    @else
                        ...
                        <div id="siswa-list" class="max-h-72 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg divide-y divide-gray-100 dark:divide-gray-700">
                            @foreach($siswa as $s)
                                <label class="siswa-item flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer"
                                    data-name="{{ strtolower($s->nama_siswa) }}"
                                    data-nisn="{{ $s->nisn }}">
                                    <input type="checkbox" name="siswa_id[]" value="{{ $s->id_siswa }}"
                                        class="rounded border-gray-300 text-purple-600 focus:ring-purple-500" />
                                    <div>
                                        <p class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ $s->nama_siswa }}</p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">NISN: {{ $s->nisn }}</p>
                                    </div>
                                </label>
                            @endforeach
                        </div>
