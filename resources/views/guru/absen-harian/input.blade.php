<x-layouts.admin>
    <x-slot:title>Absen Harian</x-slot:title>
    ...
    <div class="container px-6 mx-auto">
        <div class="my-6">
            ...
            <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">Absen Harian</h2>
        </div>
        ...
        <div class="mb-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-xs dark:shadow-none dark:border dark:border-gray-700">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                    ...
                    <p class="font-semibold text-gray-700 dark:text-gray-200">{{ $kelas->nama_kelas }}</p>
                </div>
                <div>
                ...
                <div>
                    <p class="text-gray-500 dark:text-gray-400">Cover Mapel</p>
                    <p class="font-semibold text-gray-700 dark:text-gray-200">{{ $mapelList->implode(', ') }}</p>
                </div>
                <div>
                    <p class="text-gray-500 dark:text-gray-400">Jumlah Jadwal</p>
                    <p class="font-semibold text-gray-700 dark:text-gray-200">{{ $jadwalHarian->count() }} jam</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xs dark:shadow-none dark:border dark:border-gray-700 overflow-hidden">
            <form method="POST" action="{{ route('guru.absen-harian.store', $kelas->id_kelas) }}">
                @csrf
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/70">
                                ...
                                <td class="px-5 py-3">
                                    <select name="absensi[{{ $reg->id_registrasi }}][status]"
                                        ...
                                        @foreach(['Hadir','Sakit','Izin','Alpa','Terlambat','Bolos'] as $status)
                                            <option value="{{ $status }}"
                                                {{ ($absensiHarian[$reg->id_registrasi] ?? 'Hadir') == $status ? 'selected' : '' }}>
                                                {{ $status }}
                                            </option>
                                        @endforeach
                                    </select>
